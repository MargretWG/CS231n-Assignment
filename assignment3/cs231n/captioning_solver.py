...
class CaptioningSolver(object):
  ...
  def __init__(self, model, data, **kwargs):
    ...
    if not hasattr(optim, self.update_rule):#查看optim.py中是否有输入的更新规则
  ...
  def train(self):
    ...
    """
    num_train = self.data['train_captions'].shape[0]#样本数N
    iterations_per_epoch = int(max(num_train / self.batch_size, 1))#每轮要训练的批数
    num_iterations = int(self.num_epochs * iterations_per_epoch)#总批数
    ...
    for t in range(num_iterations):
      ...
      # TODO: Implement some logic to check Bleu on validation set periodically
          '''minibatch = sample_coco_minibatch(self.data, split='val', batch_size=self.batch_size)
