...
def print_tuple(x):
  for item in x:
    if type(item)==tuple:
      ...
      print ('    C:', item.shape, np.sum(np.absolute(item)))
    else:
      print ('    C:', item)
...
class PretrainedCNN(object):
  ...
  def backward(self, dout, cache):
    ...
    for i in reversed(range(start, end + 1)):
      if self.debug:
        print ('layer:', i+1, "top gradient's shape and abs sum:", dnext_a.shape, np.sum(np.absolute(dnext_a)))

      i1 = i + 1
      if i == len(self.conv_params) + 1:
        cache = layer_caches.pop()
        # This is the last fully-connected layer
        dprev_a, dw, db = affine_backward(dnext_a, cache)
        grads['W%d' % i1] = dw
        grads['b%d' % i1] = db
      elif i == len(self.conv_params):
        # This is the fully-connected hidden layer
        cache = layer_caches.pop()
        temp = affine_bn_relu_backward(dnext_a, cache)
        ...
        # This is a conv layer
        cache = layer_caches.pop()
        temp = conv_bn_relu_backward(dnext_a, cache)
        dprev_a, dw, db, dgamma, dbeta = temp
        grads['W%d' % i1] = dw
        grads['b%d' % i1] = db
        grads['gamma%d' % i1] = dgamma
      ...
      if self.debug:
        names = []
        for name, item in grads.items():
          print (name, '->G', item.shape, np.sum(np.absolute(item)))
          names.append(name)
        for name in names:
          del grads[name]
        ...
        dnext_a = dprev_a
        print ('layer:', i+1, "bottom gradient's shape and abs sum:", dnext_a.shape, np.sum(np.absolute(dnext_a)))
        #raw_input()
  ...
  def calc_loss(self, X, y):
    scores, cache = self.forward(X, mode='test')
    loss, dscores = softmax_loss(scores, y)
    dX, _ = self.backward(dscores, cache)
    return loss, np.argmax(scores), dX
