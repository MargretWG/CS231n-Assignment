...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    grad_term,grads['W2'],grads['b2']=affine_backward(ds,cache2)
    _,grads['W1'],grads['b1']=affine_relu_backward(grad_term,cache1)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores,self.cache['cache'+str(self.num_layers)]=affine_forward(out,self.params['W'+str(self.num_layers)],self.params['b'+str(self.num_layers)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(1, hidden_layers + 2):
      sum += np.sum(np.square(self.params['W' + str(i)]))
    loss = sum * 0.5 * self.reg
    loss_0, dx = softmax_loss(scores, y)#softmax层反向传播
    loss += loss_0

    grad_term,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)]=affine_backward(dx,self.cache['cache'+str(self.num_layers)])#softmax前面一层的反向传播
    grads['W' + str(self.num_layers)] += self.reg * self.params['W' + str(self.num_layers)]
    if not self.use_batchnorm:
      for i in range(1,self.num_layers)[::-1]:
        if self.use_dropout:
          grad_term = dropout_backward(grad_term, self.cache['drop' + str(i)])
        grad_term, grads['W' + str(i)], grads['b' + str(i)] = affine_relu_backward(grad_term, self.cache['cache' + str(i)])
        grads['W' + str(i)] += self.reg * self.params['W' + str(i)]
    else:
      for i in range(1,self.num_layers)[::-1]:
        if self.use_dropout:
          grad_term = dropout_backward(grad_term, self.cache['drop' + str(i)])
        grad_term,grads['W'+str(i)],grads['gamma'+str(i)],grads['beta'+str(i)]=affine_bn_relu_backward(grad_term,self.cache['cache'+str(i)])
        grads['W' + str(i)] += self.reg * self.params['W' + str(i)]
