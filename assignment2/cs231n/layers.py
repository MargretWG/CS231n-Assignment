...
def affine_backward(dout, cache):
  ...
  #############################################################################
  db=np.sum(dout,axis=0) #(N,) here we do not divide
  x_N=x.reshape(x.shape[0],-1)#(N,D)
  dw=np.dot(x_N.T,dout) #(D,M)
...
def batchnorm_backward(dout, cache):
  ...
  #易错！ sample_mean,sample_var是关于x的函数！
  m=sample_var+eps
  dx_normalized=dout*gamma #(N,D)
  m=1/np.sqrt(sample_var+eps) #(D,)
  dsample_var = -1.0 / 2 * (m**3) * np.sum(dx_normalized * (x - sample_mean), axis=0) #(D,)
  dsample_mean=np.sum(dx_normalized,axis=0)*(-m)
  dx=m*dx_normalized+dsample_var*(2.0/N)*(x-sample_mean)+dsample_mean/N
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = dout*gamma*((N-1.0)/(N*(sample_var+eps)**2) - (x-sample_mean)*(-sample_mean+sample_mean/N)/((N*(sample_var+eps)**(3.0/2.0))))
  print ('I am exhausted!! Still can not get right answer. Maybe next time')'''
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    shape=x.shape[1]
    num=int(shape*p)
    mask=np.random.rand(*x.shape)<((1-p)+1e-5)
    out=x*mask
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
